    ...
    for (let m = 0; m < col.length; m++) {
        ...
        for (let i = 0; i < data.data.length; i++) {
            ...
            if (col[m].innerHTML === data.data[i].SEMBOL) {
                ...
                veri += `<tr>
                            <th class="p-3" title="${data.data[i].ACIKLAMA}"><i class="bi bi-info-circle"></i> ${data.data[i].SEMBOL.includes("TRY")?data.data[i].SEMBOL.slice(0,data.data[i].SEMBOL.indexOf("TRY"))+"/TRY":data.data[i].SEMBOL}</th>
